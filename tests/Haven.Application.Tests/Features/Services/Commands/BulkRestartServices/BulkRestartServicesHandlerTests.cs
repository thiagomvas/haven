...
[Category("Unit")]
public sealed class BulkRestartServicesHandlerTests
{
    private IProjectRepository _projectRepository;
    private IDeploymentJobEnqueuer _deploymentJobEnqueuer;
    private BulkRestartServicesHandler _sut;
    ...
    public void Setup()
    {
        ...
        _sut = new BulkRestartServicesHandler(_projectRepository, _deploymentJobEnqueuer);
    }
    ...
    public async Task Handle_ShouldReturnSuccessWithAllSucceeded_WhenAllServicesExist()
    {
        ...
        _deploymentJobEnqueuer.Received(1).EnqueueRestart(command.ProjectId, command.EnvironmentId, service1.Id);
        _deploymentJobEnqueuer.Received(1).EnqueueRestart(command.ProjectId, command.EnvironmentId, service2.Id);
    }
    ...
    private static BulkRestartServicesCommand CreateCommand(IReadOnlyList<Guid> serviceIds) => new()
    {
