...
public class BulkStopServicesIntegrationTests
{
    ...
    [Test]
    public async Task BulkStopServices_WithAllValidServices_ReturnsSuccessForEach()
    {
        ...
        var request = new BulkStopServicesCommand { ServiceIds = [service1Id, service2Id] };
        var response = await _fixture.Client.PostAsJsonAsync(
            $"/api/projects/{projectId}/environments/{environmentId}/services/bulk-stop",
            request);
    ...
    [Test]
    public async Task BulkStopServices_WithMixOfValidAndInvalidServices_ReturnsPartialFailure()
    {
        var (projectId, environmentId) = await CreateProjectAndEnvironmentAsync();
        var serviceId = await CreateServiceAsync(projectId, environmentId, "web");
        var missingServiceId = Guid.NewGuid();

        var request = new BulkStopServicesCommand { ServiceIds = [serviceId, missingServiceId] };
        var response = await _fixture.Client.PostAsJsonAsync(
            $"/api/projects/{projectId}/environments/{environmentId}/services/bulk-stop",
            request);
    ...
    [Test]
    public async Task BulkStopServices_WithEmptyServiceIds_ReturnsValidationError()
    {
        var (projectId, environmentId) = await CreateProjectAndEnvironmentAsync();

        var request = new BulkStopServicesCommand { ServiceIds = [] };
        var response = await _fixture.Client.PostAsJsonAsync(
            $"/api/projects/{projectId}/environments/{environmentId}/services/bulk-stop",
            request);
    ...
    [Test]
    public async Task BulkStopServices_WithInvalidEnvironmentId_ReturnsNotFound()
    {
        var (projectId, _) = await CreateProjectAndEnvironmentAsync();
        var invalidEnvironmentId = Guid.NewGuid();

        var request = new BulkStopServicesCommand { ServiceIds = [Guid.NewGuid()] };
        var response = await _fixture.Client.PostAsJsonAsync(
            $"/api/projects/{projectId}/environments/{invalidEnvironmentId}/services/bulk-stop",
            request);
