...
public class BulkRestartServicesIntegrationTests
{
    ...
    [Test]
    public async Task BulkRestartServices_WithAllValidServices_ReturnsSuccessForEach()
    {
        ...
        var request = new BulkRestartServicesCommand { ServiceIds = [service1Id, service2Id] };
        var response = await _fixture.Client.PostAsJsonAsync(
            $"/api/projects/{projectId}/environments/{environmentId}/services/bulk-restart",
            request);
    ...
    [Test]
    public async Task BulkRestartServices_WithMixOfValidAndInvalidServices_ReturnsPartialFailure()
    {
        var (projectId, environmentId) = await CreateProjectAndEnvironmentAsync();
        var serviceId = await CreateServiceAsync(projectId, environmentId, "web");
        var missingServiceId = Guid.NewGuid();

        var request = new BulkRestartServicesCommand { ServiceIds = [serviceId, missingServiceId] };
        var response = await _fixture.Client.PostAsJsonAsync(
            $"/api/projects/{projectId}/environments/{environmentId}/services/bulk-restart",
            request);
    ...
    [Test]
    public async Task BulkRestartServices_WithEmptyServiceIds_ReturnsValidationError()
    {
        var (projectId, environmentId) = await CreateProjectAndEnvironmentAsync();

        var request = new BulkRestartServicesCommand { ServiceIds = [] };
        var response = await _fixture.Client.PostAsJsonAsync(
            $"/api/projects/{projectId}/environments/{environmentId}/services/bulk-restart",
            request);
    ...
    [Test]
    public async Task BulkRestartServices_WithInvalidEnvironmentId_ReturnsNotFound()
    {
        var (projectId, _) = await CreateProjectAndEnvironmentAsync();
        var invalidEnvironmentId = Guid.NewGuid();

        var request = new BulkRestartServicesCommand { ServiceIds = [Guid.NewGuid()] };
        var response = await _fixture.Client.PostAsJsonAsync(
            $"/api/projects/{projectId}/environments/{invalidEnvironmentId}/services/bulk-restart",
            request);
