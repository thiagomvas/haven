...
public class WriteProjectOnProjectCreatedEventHandlerTests
{
    private HavenDbContext _context = null!;
    private IProjectRepository _projectRepository = null!;
    private IManifestSerializer<Project> _mockSerializer = null!;
    private WriteProjectOnProjectCreatedEventHandler _handler = null!;
    ...
    public async Task SetUp()
    {
        ...
        _handler = new WriteProjectOnProjectCreatedEventHandler(_mockSerializer, _projectRepository);
    }
    ...
    [Test]
    public async Task Handle_WithExistingProject_CallsWriteAsync()
    {
        // Arrange
        ...
        var @event = new ProjectCreatedEvent(project.Id, project.Name);

        // Act
        await _handler.Handle(@event, CancellationToken.None);

        // Assert
        await _mockSerializer.Received(1).WriteAsync(Arg.Is<Project>(p => p.Name == "Test Project"), Arg.Any<CancellationToken>());
    }

    [Test]
    public async Task Handle_WithNonexistentProject_DoesNotCallWriteAsync()
    {
        // Arrange
        var @event = new ProjectCreatedEvent(Guid.NewGuid(), "Test Project");
