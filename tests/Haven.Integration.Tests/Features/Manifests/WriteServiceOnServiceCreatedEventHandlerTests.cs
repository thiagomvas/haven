...
public class WriteServiceOnServiceCreatedEventHandlerTests
{
    ...
    private WriteServiceOnServiceCreatedEventHandler _handler = null!;
    ...
    public async Task SetUp()
    {
        ...
        _handler = new WriteServiceOnServiceCreatedEventHandler(_mockSerializer, _serviceRepository);
    }
    ...
    [Test]
    public async Task Handle_WithExistingService_CallsWriteAsync()
    {
        // Arrange
        ...
        var @event = new ServiceCreatedEvent(service.Id, service.Name);

        // Act
        await _handler.Handle(@event, CancellationToken.None);

        // Assert
        await _mockSerializer.Received(1).WriteAsync(Arg.Is<Service>(s => s.Name == "test-service"), Arg.Any<CancellationToken>());
    }

    [Test]
    public async Task Handle_WithNonexistentService_DoesNotCallWriteAsync()
    {
        // Arrange
        var @event = new ServiceCreatedEvent(Guid.NewGuid(), "test-service");
