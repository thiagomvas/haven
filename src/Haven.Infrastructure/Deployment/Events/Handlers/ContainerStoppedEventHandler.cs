...
public class ContainerStoppedEventHandler : INotificationHandler<ContainerStoppedEvent>
{
    ...
    public ContainerStoppedEventHandler(HavenDbContext db, IProjectRepository repository, ISidecarRepository sidecarRepository, ILogger<ContainerStoppedEventHandler> logger)
    {
        _db = db;
    ...
    public async ValueTask Handle(ContainerStoppedEvent notification, CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("Sidecar {SidecarId} marked as stopped", sidecar.Id);
    }
}