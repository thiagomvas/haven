...
public sealed class BulkStopServicesHandler(
    IProjectRepository projectRepository,
    IDeploymentJobEnqueuer deploymentJobEnqueuer)
    : ICommandHandler<BulkStopServicesCommand, BulkServiceActionResponse>
{
    public async ValueTask<Result<BulkServiceActionResponse>> Handle(
        BulkStopServicesCommand request,
        CancellationToken cancellationToken)
    {
        ...
        foreach (var serviceId in request.ServiceIds)
        {
            ...
            try
            {
                deploymentJobEnqueuer.EnqueueStop(request.ProjectId, request.EnvironmentId, serviceId);
